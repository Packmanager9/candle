...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Circle {
        ...
        draw() {
            ...
            // this.x = 350 + (300 * Math.sin(this.phi) * Math.cos(this.theta));
            // this.y = 350 + (300 * Math.sin(this.phi) * Math.sin(this.theta));
            // this.z = 350 + (300 * Math.cos(this.phi));



            canvas_context.lineWidth = this.strokeWidth
            canvas_context.strokeStyle = this.color//+(this.slap.toString(16))
            canvas_context.beginPath();
            if (this.radius > 0) {
                canvas_context.arc(this.x, this.y, this.radius, 0, (Math.PI * 2), true)
                canvas_context.fillStyle = this.color//+(this.slap.toString(16))
                canvas_context.fill()
                canvas_context.stroke();
            } else {
    ...
    function setUp(canvas_pass, style = "#36393E") {
        canvas = canvas_pass
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Candle{
        constructor(){
            this.stalk = new Rectangle(325,350, 50, 400, "#FFFFDD")
            this.wick = new Rectangle(347.5,330, 5, 400, "#333333")
            this.cherry = new Rectangle(347.5,330, 5, 5, "#FFAA00")
            this.time = 0
            this.flame = []
            this.counter = 0
            this.metaangle = 0
        ...
        draw(){
            ...
            this.cherry.draw()
            this.stalk.draw()
            this.stalk.y = 350 + this.time
            this.wick.y = 330 + this.time
            this.cherry.y = 330 + this.time
            this.counter++


            if(Math.random() < this.flicker*10){
                this.height =-((Math.random()*.2)+.15)
            }
            if(Math.random() < this.flicker*150){
                this.metaangle = Math.random()*Math.PI*2
            }

            if(this.counter%9 == 0){
                let spark = new Circle(350, this.wick.y+5, 3, "#FFAA0020", Math.cos(this.metaangle)*.25, this.height)
                spark.angle = 0
                spark.friction = .99
                this.flame.unshift(spark)
            }
            for(let t = 0;t<this.flame.length;t++){
                this.flame[t].ymom = this.height
                this.flame[t].angle += .01
                this.flame[t].radius = Math.max(3+(Math.cos(this.flame[t].angle)*5), 0)
                this.flame[t].xmom += (350-this.flame[t].x)/100
                this.flame[t].frictiveMove()
                this.flame[t].draw()

                if(this.flame[t].radius < 7.5 || this.flame[t].y < (this.wick.y-2)){
                    this.flame[t].color = "#FFAA0040"
                }else{
                    this.flame[t].color = "#0066ff20"
                }

                if(this.flame[t].radius == 0){
                    this.flame[t].marked = 1
                }
            }
            for(let t = 0;t<this.flame.length;t++){
                if(this.flame[t].marked ==1){
                    this.flame.splice(t,1)
                }
            }
        }
    }
    
    let windlet = new Candle()
